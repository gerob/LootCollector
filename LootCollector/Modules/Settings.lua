...
local function ensureDefaults()
    ...
    if c.mapFilters.maxReqLevel == nil then c.mapFilters.maxReqLevel = 0 end
		...
		args = {
			...
			sharing = {
				...
				args = {
					...
					versionDesc = {
						type = "description",
						name = "Optionally block incoming discoveries from players running older or specific versions of LootCollector. This is useful for filtering out coordinates generated by outdated addons with known bugs.",
						order = 26,
					},
					minVersionGateOverride = {
						type = "select",
						name = "Minimum Version Gate",
						desc = "Discoveries from players using addon versions older than this will be automatically dropped.",
						order = 27,
						values = function()
							local values = {}
					...
					versionFilters = {
						...
						desc = "Uncheck versions to automatically block all incoming discovery data from players running that version.",
						order = 28,
						values = function()
							local values = {}
							if L.db.profile.sharing.seenVersions then
					...
					selectAllVersions = {
						type = "execute",
						name = "Select All",
						desc = "Check all versions to mark them as trusted.",
						order = 28.1,
						width = "half",
						func = function()
							if L.db.profile.sharing.blockedVersions then
								wipe(L.db.profile.sharing.blockedVersions)
							end
						end,
					},
					deselectAllVersions = {
						type = "execute",
						name = "Deselect All",
						desc = "Uncheck all versions to block them.",
						order = 28.2,
						width = "half",
						func = function()
							L.db.profile.sharing.blockedVersions = L.db.profile.sharing.blockedVersions or {}
