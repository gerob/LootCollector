...
function Comm:CalculateSharingInterval(mpm)
    ...
    if mpm <= 100 then
        return baseInterval
    elseif mpm >= 15000 then
        return maxInterval
    else
        local ratio = (mpm - 100) / 14900
        return baseInterval + ratio * (maxInterval - baseInterval)
    end
end

function Comm:GetCurrentSharingInterval()
    if not self._cachedSharingInterval then
        local mpm = self:GetCurrentChannelTrafficRate()
        self._cachedSharingInterval = self:CalculateSharingInterval(mpm)
    end
    return self._cachedSharingInterval
end
...
function Comm:EnqueueOutgoingSync(wire)
    local p = L and L.db and L.db.profile
    if not (p and p.sharing and p.sharing.enabled and p.sharing.publicChannelEnabled) then
        return
    end
    ...
    if not op or (op ~= "DISC" and op ~= "CONF") then
        return
    end
...
function Comm:EnsureChannelJoined()
    ...
    local p = L and L.db and L.db.profile
    if not (p and p.sharing and p.sharing.enabled and p.sharing.publicChannelEnabled) or (L.IsZoneIgnored and L:IsZoneIgnored()) then 
...
function Comm:OnUpdate(elapsed)
    ...
    self._trafficUpdateTimer = (self._trafficUpdateTimer or 0) + elapsed
    if self._trafficUpdateTimer >= 2.0 then
        self._trafficUpdateTimer = 0
        self._cachedTrafficRate = self:GetCurrentChannelTrafficRate()
        self._cachedSharingInterval = self:CalculateSharingInterval(self._cachedTrafficRate or 0)
        
        local mpm = self._cachedTrafficRate or 0
        local p = L and L.db and L.db.profile
        local publicEnabled = p and p.sharing and p.sharing.publicChannelEnabled and p.sharing.enabled
        local autoPause = p and p.sharing and p.sharing.autoPauseEnabled
        local threshold = p and p.sharing and p.sharing.autoPauseThreshold or 5000
        
        if autoPause and mpm > threshold and not self._publicChannelAutoPaused and publicEnabled then
            self._publicChannelAutoPaused = true
            self._autoPauseRejoinTime = now() + 300
            self:LeavePublicChannel()
    ...
    if self._publicChannelAutoPaused then
        local p = L and L.db and L.db.profile
        local autoPause = p and p.sharing and p.sharing.autoPauseEnabled
        if not autoPause or now() >= (self._autoPauseRejoinTime or 0) then
            self._publicChannelAutoPaused = false
...
local function _onChatMsgChannel(_, _, msg, sender, _, _, _, _, _, _, channelName)
    ...
    if channelName then
        ...
        local chB = string.upper(Comm.channelName or "")
        if chA == chB then
            Comm._incomingTimestamps = Comm._incomingTimestamps or {}
            table.insert(Comm._incomingTimestamps, now())
        end
    end
...
function Comm:_ProcessChatMsg(msg, sender, channelName)
    ...
    if optOp and optMid then
        ...
        if optMid and Core and Core.FindByMid then
            local existing = Core:FindByMid(optMid)
            if existing then
                if optOp == "DISC" or (optOp == "CONF" and existing.s == "CONFIRMED") then
                    if pTime then L:ProfileStop("Comm:_ProcessChatMsg", pTime) end
                    return
                end
            end
...
function Comm:RouteIncoming(tbl, via, sender)
    ...
    -- Version check notifications disabled to allow community updates without in-game alerts or spoofing.
