...
function Core:RunUnifiedDatabasePass(blacklistName)
    ...
    for guid, d in pairs(discoveries) do
        ...
        if not removeReason and isCoA and (d.dt == MS_TYPE or d.ist == 6 or d.ist == 7 or d.ist == 8 or d.ist == 9) then
            removeReason = "coa"
        end
...
local function FindWorldforgedInZone(continent, zoneID, itemID, db)
    ...
    local targetName = select(1, GetItemInfo(itemID))
    if not targetName and itemID then
        local link = select(2, GetItemInfo(itemID))
        if link then targetName = link:match("%[(.+)%]") end
    end
            ...
            if d and d.c == continent then
                ...
                if Constants and d.dt == Constants.DISCOVERY_TYPE.WORLDFORGED then
                    ...
                    elseif targetName then
                        local dName = (d.il and d.il:match("%[(.+)%]")) or select(1, GetItemInfo(d.i))
                        if dName and string.lower(dName) == string.lower(targetName) then
                            if pTime then L:ProfileStop("Core:FindWorldforgedInZone", pTime) end
                            return d
                        end
                    end
    ...
    for _, d in pairs(db) do
        if d and d.c == continent and d.z == zoneID then
            local Constants = L:GetModule("Constants", true)
            if Constants and d.dt == Constants.DISCOVERY_TYPE.WORLDFORGED then
                if d.i == itemID then
                    if pTime then L:ProfileStop("Core:FindWorldforgedInZone", pTime) end
...
function Core:DeduplicateItems(mysticScrollsKeepOldest)
    ...
    for guid, d in pairs(discoveries) do
        if d and d.i and d.z and d.c ~= nil then
            local name = (d.il and d.il:match("%[(.+)%]")) or select(1, GetItemInfo(d.i))
            local itemKey = name and string.lower(name) or tostring(d.i)
            
            local key = d.c .. ":" .. d.z .. ":" .. itemKey
...
function Core:UpdateItemRecordFromCache(itemID)
    ...
    for _, d in pairs(discoveries) do
        if type(d) == "table" and d.i == itemID then
            ...
            local isCoA = self.IsConfirmedCoARealm and self:IsConfirmedCoARealm()
            local MS_TYPE = Constants and Constants.DISCOVERY_TYPE.MYSTIC_SCROLL or 2
            if isCoA and (d.dt == MS_TYPE or d.ist == 6 or d.ist == 7 or d.ist == 8 or d.ist == 9) then
                discoveries[_] = nil
                updated = true
            end
...
function Core:OnGetItemInfoReceived(itemID, success)
    local pTime = L.ProfileStart and L:ProfileStart() 
    ...
    if itemID then
        ...
        if mapTooltip and mapTooltip:IsShown() and mapTooltip:GetOwner() and mapTooltip:GetOwner():GetParent() == WorldMapButton then
            ...
            if discovery and tonumber(discovery.i) == tonumber(itemID) then
                L._debug("Core-Cache", "Forcing map tooltip refresh for newly cached item: " .. tostring(itemID))
...
function Core:AddDiscovery(d, options)
    ...
    if isCoA then
        local MS_TYPE = Constants and Constants.DISCOVERY_TYPE.MYSTIC_SCROLL or 2
        local ist = tonumber(d.ist) or 0
        if ist == 0 and Constants.ITEM_SUBTYPE_TO_ID then
            local _, _, _, _, _, _, itemSubType = GetItemInfo(itemID)
            if itemSubType then
                ist = Constants.ITEM_SUBTYPE_TO_ID[itemSubType] or 0
            end
        end
        if dt == MS_TYPE or ist == 6 or ist == 7 or ist == 8 or ist == 9 then
            L._debug("Core-Block", "Blocked incoming discovery incompatible with CoA realm: " .. tostring(itemID))
            if pTime then L:ProfileStop("Core:AddDiscovery", pTime) end
            return nil
        end
    end
    ...
    if (time() - t0) >= 120 * 86400 then
        L._debug("Core-Block", "Blocked incoming expired discovery: " .. tostring(itemID) .. " (t0=" .. tostring(t0) .. ")")
        if pTime then L:ProfileStop("Core:AddDiscovery", pTime) end
        return nil
    end
